...
import estimatesData from './mockData/estimates.json'

export const fetchUserByOpportunityId = (opportunityId) => fakeCall(customerData)

export const fetchOpportunityInfo = opportunityId => fakeCall(opportunityInfoData)

export const fetchIndividualEstimate = (opportunityId, estimateId) => fakeCall(individualEstimateData)

export const fetchEstimates = opportunityId => fakeCall(estimatesData)

export const validateAccessCode = (opportunityId, accessCode) => {
    const mockResult = (accessCode === '111111') ? {
      token: "JWT_token_here",
      opportunityId: "fake",
      expiresIn: "60"
    }: (accessCode === '222222') ? Promise.reject(`Access code ${accessCode} is expired`): Promise.reject(`Access code ${accessCode} is invalid`)
